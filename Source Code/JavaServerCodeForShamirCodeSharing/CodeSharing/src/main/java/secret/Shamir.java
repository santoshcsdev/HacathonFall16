...
public class Shamir {
    ...
    public static SecretShare[] split(final BigInteger secret, int needed, int available,
                                      BigInteger prime, Random random) {
        ...
        for (int x = 1; x <= available; x++) {
            ...
            for (int exp = 1; exp < needed; exp++) {
                accum = accum.add(coeff[exp].multiply(BigInteger.valueOf(x).pow(exp).mod(prime)))
                        .mod(prime);
            }
    ...
    public static BigInteger combine(final SecretShare[] shares, final BigInteger prime) {
        ...
        for (int formula = 0; formula < shares.length; formula++) {
            ...
            for (int count = 0; count < shares.length; count++) {
                ...
                numerator = numerator.multiply(BigInteger.valueOf(nextposition).negate()).mod
                        (prime); // (numerator * -nextposition) % prime;
                denominator = denominator.multiply(BigInteger.valueOf(startposition -
                        nextposition)).mod(prime); // (denominator * (startposition -
                // nextposition)) % prime;
            }
    ...
    private static BigInteger modInverse(BigInteger k, BigInteger prime) {
        k = k.mod(prime);
        BigInteger r = (k.compareTo(BigInteger.ZERO) == -1) ? (gcdD(prime, k.negate())[2]).negate
                () : gcdD(prime, k)[2];
    ...
    public static void main(final String[] args) {
        ...
        sharesToViewSecret = new SecretShare[]{shares[1]}; // 1
        result = Shamir.combine(sharesToViewSecret, prime);
        System.out.println(result.equals(secret));*/
    }
}